    ...
    return {
        link: function(scope, element) {
            var listener = function(event, toState, toParams, fromState, fromParams) {
                ...
                if (toState.data && toState.data.pageTitle) title = '流量管理平台 | ' + toState.data.pageTitle;
                $timeout(function() {
                    element.text(title);
                });
            };
 ...
 */
function icheck($timeout, common) {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function($scope, element, $attrs, ngModel) {
            return $timeout(function() {
                ...
                // 监听 ngChecked 变化，更新 checked icon & ngModel
                /*$scope.$watch($attrs['ngChecked'], function(newValue) {
                    $(element).iCheck('update');
                    if (newValue) {
                        $(element).trigger('ifChanged');
                    }
                })*/
                ...
                return $(element).iCheck({
                    ...
                    if ($(element).attr('type') === 'checkbox' && $attrs['ngModel']) {
                        return common.safeApply(function() {
    ...
    return {
        ...
        controller: function($scope) {
            ...
            function serverData(sSource, aoData, fnCallback, oSettings) {
                ...
                (oSettings.jqXHR.$promise || oSettings.jqXHR).then(function(response) {
        ...
        link: function(scope, element, attr) {
            ...
            var displayWatch = scope.$watch(function() {
                setTimeout(function(){
                    ...
                    if (newValue !== 'none') {
                        $wrapper.css('min-height', element.parent().height() + element.height() + 70);
                    } else {
