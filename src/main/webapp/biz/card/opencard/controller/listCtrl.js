(function(angular){
	...
	function openCardListCtrl($scope, $http, DTOptionsBuilder, DTColumnBuilder,openCardService,spService,pkgService){
		//搜索关键字
		$scope.openCardQuery={
				keyWork:'',
				activeDaysSmall:'',
		...
		// 定义DataTables选项
		$scope.dtInstance = {
				serverData : openCardService.getOpenCardList
		};
		...
		function setCardStatus(e, dt, node, config){
			//0=可测试/1=库存卡/3=可激活/4=已激活/5=已停卡/6=已销卡
			if(node.cardStatus==0){
				...
				return '<span style="color:red";>已停机</span>';
		...
		$scope.exportOpenCardFile=function(){
			...
			openCardService.exportOpenCardFile(openCardQuery,function(data){
	...
	angular.module('card')
	.controller('openCardListCtrl', openCardListCtrl)
})(angular);