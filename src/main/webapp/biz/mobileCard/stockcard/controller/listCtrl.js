(function(angular) {
	function stockCardListCtrl($scope, $http, DTOptionsBuilder, DTColumnBuilder,
			$state, $rootScope, $compile, flowcardService, $templateRequest,
		...
		// 定义DataTables选项
		$scope.dtInstance = {
				serverData : flowcardService.getLifeCycleAll
		};
		...
		$scope.dtColumns = [
				...
				DTColumnBuilder.newColumn('openTime').withOption('width',64).withTitle('开户时间')];
		...
		$scope.lifeCycleHighSearchForm = function() {
			...
			//移动卡开户时间
			if (lifeCycleQuery.openTime && lifeCycleQuery.openTime.startDate
					&& lifeCycleQuery.openTime.endDate) {
				var startTime = lifeCycleQuery.openTime.startDate
						.format('YYYY-MM-DD 00:00:00');
				var endTime = lifeCycleQuery.openTime.endDate
						.format('YYYY-MM-DD 23:59:59');
				lifeCycleQuery.openStarttime = startTime;
				lifeCycleQuery.openEndtime = endTime;
			}
			lifeCycleQuery.openTime = "";
		...
		$scope.resetForm = function(id, state) {
			...
			lifeCycleQuery.openTime = null;
		};
		
		// 打开卡出库窗口
		$scope.openStockCard = function() {
			var modalInstance = $uibModal.open({
				templateUrl : 'biz/mobileCard/stockcard/view/create.html',
				controller : 'createStockCardCtrl',
				resolve : {
					param : function() {
						return {};
					}
		...
		$scope.exportLifeCycleFile = function() {
			...
			flowcardService.exportLifeCycleFile(lifeCycleQuery, function(data) {
	...
	//卡出库操作
	function createStockCardCtrl($scope,$uibModalInstance,flowcardService,FileUploader) {
		...
		var uploaderStockcards = $scope.uploaderStockcards = new FileUploader({
			url : "stock/lifecycle/importStockcard",
		...
		$scope.stockCard = function(status) {
			if(returnKey == ""){
				...
				flowcardService.stockCard({stockKey:returnKey},function(data) {
					if (data.result) {
						swal({
							title: "",
							text: "批量出库操作成功!",
	...
	angular.module('mobileCard')
		.controller('stockCardListCtrl', stockCardListCtrl)
		.controller('createStockCardCtrl', createStockCardCtrl);
})(angular);
