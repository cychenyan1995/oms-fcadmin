(function(angular) {
	...
	function flowPackageListCtrl($scope, DTOptionsBuilder, DTColumnBuilder,$filter,pkgService,spService,opService,$uibModal) {
		...
		$scope.dtInstance = {
				/*serverData : function(param){
	            	...
	            	return pkgService.getFlowPackageAll(param);
	            }*/
				serverData : pkgService.getFlowPackageAll
		};
		...
		$scope.dtColumns = [
				...
				DTColumnBuilder.newColumn('emptyPk').withOption('width',columnWidth).withOption('ellipsis',true).withTitle('套餐分类').renderWith(emptyPk),
		...
		function packageName(e, dt, node, config) {
			if(node.packageName != null){
				return operateHtml = '<a href="javascript:;" ng-click="flowPackageDetail(item);">'+ node.packageName +'</a>';
			}else{
				return "";
			}
		}
		...
		function emptyPk(e, dt, node, config){
			if(node.emptyPk == 0){
				return '开卡套餐';
			}else{
				return '白卡套餐';
		...
		$scope.openCreateFlowPackageWindow = function() {
			var modalInstance = $uibModal.open({
				templateUrl : 'biz/pkg/flowpackage/view/create.html',
				controller : 'createFlowPackageCtrl',
				// size : 'lg',
				resolve : {
					param : function() {
						return {};
		...
		$scope.flowPackageDetail = function(item) {
			var modalInstance = $uibModal.open({
				templateUrl : 'biz/pkg/flowpackage/view/detail.html',
				controller : 'flowPackageDetailCtrl',
                //size : 'lg',
				resolve : {
					param : function() {
						//把参数id传递给编辑页面edit.html，在editCtrl注入
...
//				$scope.dtInstance.query({});
	...
	function flowPackageDetailCtrl($scope,$uibModalInstance,param){
	...
	angular.module('pkg')
		.controller('flowPackageListCtrl', flowPackageListCtrl)
		.controller('flowPackageDetailCtrl',flowPackageDetailCtrl);
})(angular);
